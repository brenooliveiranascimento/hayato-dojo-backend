...
`;
        ...
        dojoAlunos.forEach(
          ({ nome, peso, categoria, categoriaKata, idade }, index) => {
            const categoriaKataResponse = categorias.kata.find(
              ({ codigo }) => codigo === categoriaKata
            );
            const categoriaShiaiResponse = categorias.kumite.find(
              ({ codigo }) => codigo === categoria
            );
...
_Dojo Hayato - Tradição e Excelência_`;
        ...
        acc[currObj.nome] = {
          ...
          apiLink: `https://api.whatsapp.com/send?text=${encodeURIComponent(
            currMessage
          )}`,
        };

        return acc;
      }, {} as { [key: string]: { message: string; whatsappLink: string; apiLink: string } });

      const filteredMessages = Object.fromEntries(
        Object.entries(messages).filter(([_, data]) => data.message.length > 0)
      );
      ...
      const response = {
        dojos: Object.keys(filteredMessages).map((dojoNome) => ({
          ...
          totalAlunos:
            filteredMessages[dojoNome].message.match(/\*TOTAL:\* (\d+)/)?.[1] ||
            "0",
        })),
        totais: {
