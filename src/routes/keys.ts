  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        // const atletas = await alunoRepository.find();
        ...
        // Debug: Log dos atletas recuperados
        ...
        // Debug: Verificar se as categorias estão sendo encontradas
        atletas.forEach((atleta, index) => {
          const categoriaKumite = findCategoria("kumite", atleta.categoria);
          const categoriaKata = findCategoria("kata", atleta.categoriaKata);

          console.log(`Atleta ${index + 1} - ${atleta.nome}:`);
          console.log(
            `  - Categoria Kumite ID: ${atleta.categoria} -> Encontrada: ${
              categoriaKumite ? "SIM" : "NÃO"
            }`
          );
          console.log(
            `  - Categoria Kata ID: ${atleta.categoriaKata} -> Encontrada: ${
              categoriaKata ? "SIM" : "NÃO"
            }`
          );
        });
        ...
        // Debug: Log dos grupos formados
        console.log(
          "Grupos Kumite formados:",
          Object.keys(kumiteGroups).length
        );
        console.log("Grupos Kata formados:", Object.keys(kataGroups).length);
        ...
        for (const [key, group] of Object.entries(kumiteGroups)) {
          console.log(
            `\nProcessando grupo Kumite: ${key} com ${group.atletas.length} atletas`
          );

          if (group.atletas.length > 1) {
            // Só criar bracket se houver mais de 1 atleta
            const rounds = createTournamentBracket(
              group.atletas,
              group.categoria
            );
            kumiteBrackets.push({
              categoriaInfo: {
                tipo: "kumite",
                categoria: group.categoria.categoria,
                genero: group.categoria.genero,
                faixa: group.categoria.faixa,
                peso: group.categoria.descricaoPeso,
                totalAtletas: group.atletas.length,
              },
              rounds: rounds,
            });
          } else if (group.atletas.length === 1) {
            console.log(`  - Apenas 1 atleta neste grupo, não criando bracket`);
          }
        }

        // Processar categorias de Kata
        for (const [key, group] of Object.entries(kataGroups)) {
          console.log(
            `\nProcessando grupo Kata: ${key} com ${group.atletas.length} atletas`
          );
